...
add_image_size('loopThumb', 70, 70, true);
add_image_size('relatedThumb', 250, 180, true);

/*******************************
 Колличество символов на главной
********************************/
...
function alltuts_theme_page ()
{
	...
	{
		$options = array ('logo_img', 'logo_alt','ads','advertise','contact_email','contact_text','cufon','linkedin_link','twitter_user','latest_tweet','number_tweets','facebook_link','rss_link','google_link','vk_link','keywords','description','analytics','popular_posts', 'copyright','show','strup','showsticky','popposts');
...
<form method="post" action="">
	...
	<table class="form-table">
		...
		<tr valign="top">
			<th scope="row"><label for="logo_img">Установить логотип (Должна быть прямая ссылка на изображение, а также точные размеры 133 на 34 пикселя)</label></th>
			<td>
...
<fieldset style="border:1px solid #ddd; padding-bottom:20px; margin-top:20px;">
	<legend style="margin-left:5px; padding:0 5px;color:#2481C6; text-transform:uppercase;"><strong>Основные настройки</strong></legend>
	<table class="form-table">
		<!-- General settings -->
...
<tr valign="top">
			<th scope="row"><label for="strup">Включить анимацию?</label></th>
			<td>
                ...
                <em>Включение анимации при загрузке страниц блога. По умолчанию анимация отключена</em>
			</td>
		</tr>

<tr valign="top">
			<th scope="row"><label for="popposts">Включить популярные посты?</label></th>
			<td>
				<select name="popposts" id="popposts">		
					<option value="no" <?php if(get_option('alltuts_popposts') == 'no'){?>selected="selected"<?php }?>>Нет</option>
                    <option value="yes" <?php if(get_option('alltuts_popposts') == 'yes'){?>selected="selected"<?php }?>>Да</option>
				</select> <br />
                <em>Отключение и включение популярных постов в правой колонке блога</em>
			</td>
		</tr>
...
<tr valign="top">
			<th scope="row"><label for="vk_link">Ссылка на страницу подписки по e-mail</label></th>
			<td>
...
<br><em>* Чтобы страница контактов работала, Вам нужно задать её в "Атрибутах страницы"</em>
	...
	<fieldset style="border:1px solid #ddd; padding-bottom:20px; margin-top:20px;">
	<legend style="margin-left:5px; padding:0 5px; color:#2481C6;text-transform:uppercase;"><strong>Низ сайта (Footer)</strong></legend>
		<table class="form-table">
        
        <tr>
			<th><label for="copyright">Не большой текст копирайта</label></th>
			<td>
				...
				<em>* Вы можете использовать HTML для ссылок</em>
			</td>
		</tr>
